...
export default function ActionButtonPage() {
  ...
  return (
    <ClientLayout>
      <AdminCampaignComposer title="Action Button Campaign" breadcrumb="WhatsApp Bulk / Action Button" />

      <div className="client-page-head">
        <div>
          <h1>Action Button History</h1>
          <p>Button campaigns with call-to-action interactive options.</p>
        </div>
      </div>

      {loading ? <LoadingSpinner /> : campaigns.length === 0 ? (
        <EmptyState message="No action button campaigns yet. Create one using the composer above." />
      ) : (
