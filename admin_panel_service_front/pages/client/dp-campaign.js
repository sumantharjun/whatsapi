...
export default function DPCampaignPage() {
  ...
  const load = () => api.campaigns.list().then((r) => setCampaigns((r.campaigns || []).filter((c) => c.type === 'dp')));
  ...
  return (
    <ClientLayout>
      <AdminCampaignComposer title="DP Campaign" breadcrumb="WhatsApp Bulk / DP Campaign" />

      <div className="client-page-head">
        <div>
          <h1>DP Campaign History</h1>
          <p>Display picture campaigns — bulk WhatsApp DP updates.</p>
        </div>
      </div>

      {loading ? <LoadingSpinner /> : campaigns.length === 0 ? (
        <EmptyState message="No DP campaigns yet. Create one using the composer above." />
      ) : (
